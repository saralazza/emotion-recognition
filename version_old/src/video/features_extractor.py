...
def feature_from_Video(detector,frame_video_list):
    """Extracts all the features from the input video

    Args: 
        detector: file detector for image processing
        frame_video_list: (numpy.Array) frame list 

    Returns: 
        (list) features extracted by the video
...
def annotated_From_image(image):
    ...
    blackimage = np.zeros(shape=rgb_image.shape)
    annotated_image = draw_landmarks_on_image(blackimage, detection_result)
